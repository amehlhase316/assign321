...
//		open();
	...
	public void submitClicked() {
		...
		if (!gameStarted) {
			...
			if (status == 3 && input.equals("leader")){
				System.out.println("Send a leader");
				outputPanel.appendOutput("   " + input);
				currentMessage = "{'type': 'leader', 'clientID': '"+ this.id +"'}";
			} else if (status == 3 && input.equals("start")){
				System.out.println("Send a start");
				outputPanel.appendOutput("   " + input);
				currentMessage = "{'type': 'startGame', 'clientID': '"+ this.id +"'}";
			}
			// else if (status == 3 && (!input.equals("start") && !input.equals("leader"))){
			// 	outputPanel.appendOutput("   " + input);
		...
		try {
			...
			if (json.getString("type").equals("question")) {
				...
				outputPanel.appendOutput(json.getString("value"));
				status = 4;
			} else if (json.getString("type").equals("eval")) {
				outputPanel.setPoints(json.getInt("points"));


				if (json.getString("value").equals("wrong")) {
					outputPanel.appendOutput("WRRRRRONG");
					errors++;

				} else if (json.getString("value").equals("won")) {
					outputPanel.appendOutput(json.getString("text"));
					outputPanel.setPoints(json.getInt("points"));
					byte[] imageByteArray = Base64.getDecoder().decode(json.getString("image"));
				...
				outputPanel.setPoints(json.getInt("points"));
				outputPanel.appendOutput("YOU LOST, timer ran out");
				byte[] imageByteArray = Base64.getDecoder().decode(json.getString("image"));

				ByteArrayInputStream bis = new ByteArrayInputStream(imageByteArray);
				picturePanel.insertImage(bis, 0 ,0);
				if (json.getString("value").equals("correct")){
					outputPanel.appendOutput("Your answer was correct though");
					outputPanel.setPoints(json.getInt("points"));
				} else {
					outputPanel.appendOutput("Your answer was WRONG");
				}
				gameStarted = false;
				outputPanel.appendOutput("Type your name if you want to start again.");
